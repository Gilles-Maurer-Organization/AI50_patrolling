class TextBox:
    """
    A class that represents a text box with functionalities for
    handling text input and state.
    ...
    def first_input(self) -> bool:
        ...
        This property returns whether the text box is in its first
        input state, which is used to determine if the text content is
        still the default text.
    ...
    def filled(self) -> bool:
        """
        Returns a boolean indicating if the text content is not empty
        and differs from the default text.
    ...
    def add_character(self, char) -> None:
        """
        Adds a character to the text content, replacing the default
        text if necessary.
