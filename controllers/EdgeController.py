...
class EdgeController:
    """
    This class is representing a controller responsible for managing links (edges) between nodes.

    Attributes:
        _graph (Graph): A graph represented by nodes and edges, part of the Model.
        _node_controller (NodeController): A reference to the node controller.

    Methods:
        create_link(pos): Creates a link between two nodes using the (x, y) position and the previously selected node.
    """
    def __init__(self, graph: Graph, node_controller: NodeController) -> None:
        self._graph = graph
        self._node_controller = node_controller

    def create_link(self, pos: tuple[int]) -> None:
        """
        Creates a link between two nodes by identifying a node at the given (x, y) coordinates 
        and the previously selected node.
        ...
        # If a node is already selected, the second node exists, and the two nodes are not identical:
